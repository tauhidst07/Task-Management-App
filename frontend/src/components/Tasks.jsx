...
const Tasks = ({taskProps,fetchUserTasks}) => { 
    const [task,setTask] = useState(taskProps); 
  
  return (
    <div className=' rounded-md flex flex-col  my-2 md:w-[700px] p-3 m-3 shadow-md gap-y-2'> 
      <div className='ml-4 py-2 flex flex-col gap-y-3'>
          ...
          <p > <span className='font-semibold'>STATUS: </span><span className={`${task.status =="pending"?"bg-yellow-300":"bg-green-300"} rounded-md py-1 px-2 text-sm`}>{task.status}</span> </p> 
      ...
      <div className='flex w-full ml-5 gap-x-4 '> 
        ...
        onClick={async()=>{
          const res = await axios.delete(`http://localhost:3000/api/v1/task/${task._id}`,{
              headers:{
                  authorization:`Bearer ${localStorage.getItem("token")}`
              } 
            })  
